...
from uniparser.utils import (
    AiohttpAsyncAdapter,
    HTTPXAsyncAdapter,
    HTTPXSyncAdapter,
    RequestsAdapter,
    TorequestsAsyncAdapter,
    TorequestsSyncAdapter,
    fix_relative_path,
)
...
def test_css_parser():
    ...
    # =================== test css1 ===================
    result = uni.css1.parse('<a class="url" href="/">title</a>', "a.url1", "@href")
    assert result is None
    result = uni.css1.parse('<a class="url" href="/">title</a>', "a.url", "@href")
    assert result == "/"
    result = uni.css1.parse('<a class="url" href="/">title</a>', "a.url", "$text")
    assert result == "title"
    result = uni.css1.parse('<a class="url" href="/">title</a>', "a.url", "$innerHTML")
    assert result == "title"
    result = uni.css1.parse('<a class="url" href="/">title</a>', "a.url", "$html")
    assert result == "title"
    result = uni.css1.parse('<a class="url" href="/">title</a>', "a.url", "$outerHTML")
    assert result == '<a class="url" href="/">title</a>'
    result = uni.css1.parse('<a class="url" href="/">title</a>', "a.url", "$string")
    assert result == '<a class="url" href="/">title</a>'
    result = uni.css1.parse('<a class="url" href="/">title</a>', "a.url", "$self")
...
def test_selectolax_parser():
    ...
    # =================== test se1 ===================
    result = uni.se1.parse('<a class="url" href="/">title</a>', "a.url1", "@href")
    assert result == "", result
    result = uni.se1.parse('<a class="url" href="/">title</a>', "a.url", "@href")
    assert result == "/", result
    result = uni.se1.parse('<a class="url" href="/">title</a>', "a.url", "$text")
    assert result == "title", result
    result = uni.se1.parse('<a class="url" href="/">title</a>', "a.url", "$string")
    assert result == '<a class="url" href="/">title</a>', result
    result = uni.se1.parse('<a class="url" href="/">title</a>', "a.url", "$outerHTML")
    assert result == '<a class="url" href="/">title</a>', result
    result = uni.se1.parse('<a class="url" href="/">title</a>', "a.url", "$string")
    assert result == '<a class="url" href="/">title</a>', result
    result = uni.se1.parse('<a class="url" href="/">title</a>', "a.url", "$self")
    assert isinstance(result, Node), result
    result = uni.se1.parse('<a class="url" href="/">title</a>', "a.url", "$html")
    assert result == "title", result
    result = uni.se1.parse('<a class="url" href="/">title</a>', "a.url", "$innerHTML")
    assert result == "title", result
...
def test_objectpath_parser():
    ...
    # print(result)
    assert result == [{"price": 2}, {"price": 3}], result
...
def test_loader_parser():
    ...
    # print(result)
    assert result == {
        "user1": {"name": "a", "pwd": 123},
        "user2": {"name": "b", "pwd": 456},
    }
...
def test_time_parser():
    ...
    result_time_zone = uni.time.parse(
        time_string_timezone, "encode", "%Y-%m-%dT%H:%M:%S %z"
    )
...
def test_crawler_rule():
    ...
    # print(result)
    assert result == {
        "test": {"rule1": ("http://httpbin.org/get", "http://httpbin.org/get", "http")}
    }
    crawler_rule_json = crawler_rule.to_json()
    # print(crawler_rule_json)
    assert (
        crawler_rule_json
        == r'{"name": "test", "parse_rules": [{"name": "rule1", "chain_rules": [["objectpath", "JSON.url", ""], ["python", "getitem", "[:4]"], ["udf", "(context[\"resp\"].url, context[\"request_args\"][\"url\"], input_object)", ""]], "child_rules": []}], "request_args": {"url": "http://httpbin.org/get", "method": "get"}, "regex": ""}'
    )
...
def test_default_usage():
    ...
    assert (
        json_string
        == r'{"host": "httpbin.org", "crawler_rules": {"test_crawler_rule": {"name": "test_crawler_rule", "parse_rules": [{"name": "rule1", "chain_rules": [["objectpath", "JSON.url", ""], ["python", "getitem", "[:4]"], ["udf", "(context[\"resp\"].url, input_object)", ""]], "child_rules": []}], "request_args": {"url": "http://httpbin.org/get", "method": "get"}, "regex": "https?://httpbin.org/get"}}}'
    )
    ...
    # print(result)
    assert result == {
        "test_crawler_rule": {"rule1": ("http://httpbin.org/get", "http")}
    }
    # ===================== while search failed =====================
...
def test_uni_parser():
    ...
    crawler_rule = CrawlerRule("crawler_rule", "http://example.com", [parse_rule], "")
    ...
    assert (
        CrawlerRule.loads(json_string)
        == CrawlerRule.from_json(json_string)
        == crawler_rule
        == loaded_rule
    )
    ...
    result = uni.crawl(
        crawler_rule, RequestsAdapter(), url="http://httpbin.org/get?a=1"
    )
    # print(result)
    ...
    global_loop.run_until_complete(_a_test())
    ...
    assert global_loop.run_until_complete(_test()) == {"HelloWorld": {"only_req": True}}
    ...
    parse_rule = ParseRule("parse_rule", [["context", "key_not_exist", "not found"]])
    result = uni.parse(HTML, parse_rule, {"key": "hello world"})
    # print(result)
    assert result == {"parse_rule": "not found"}

    # 9. test input_callback, load json after downloading
    crawler_rule = CrawlerRule(
        "test_input_callback",
        {"url": "http://httpbin.org/get", "method": "get"},
        [
    ...
    # print(result)
    assert result == {"test_input_callback": {"rule1": "http"}}
...
def test_crawler():
    crawler = Crawler(
        storage=JSONRuleStorage.loads(
            r'{"peps.python.org": {"host": "peps.python.org", "crawler_rules": {"list": {"name":"list","request_args":{"method":"get","retry":3,"timeout":8,"url":"https://peps.python.org/","headers":{"User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/79.0.3945.130 Safari/537.36"}},"parse_rules":[{"name":"__request__","chain_rules":[["css","td:nth-of-type(3) > a.pep","@href"],["re","^/?","@https://peps.python.org/"],["python","getitem","[:3]"]],"child_rules":[]}],"regex":"^https://peps.python.org/$"}, "subs": {"name":"detail","request_args":{"method":"get","retry":3,"timeout":8,"url":"https://peps.python.org/pep-0001/","headers":{"User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/79.0.3945.130 Safari/537.36"}},"parse_rules":[{"name":"title","chain_rules":[["css","h1.page-title","$text"],["python","getitem","[0]"]],"childs":""}],"regex":"^https://peps.python.org/pep-\\d+/$"}}}}'
        )
    )
    expected_result = {
        "list": {
            ...
            "__result__": [
                {"detail": {"title": "PEP 1 – PEP Purpose and Guidelines"}},
                {"detail": {"title": "PEP 2 – Procedure for Adding New Modules"}},
                {"detail": {"title": "PEP 4 – Deprecation of Standard Modules"}},
            ],
        }
    }
    ...
    def test_sync_crawler():
        ...
        result = crawler.crawl("https://peps.python.org/")
        # print(result)
        assert result == expected_result, result

    def test_async_crawler():
        async def _test():
            result = await crawler.acrawl("https://peps.python.org/")
            # print(result)
            assert result == expected_result

        global_loop.run_until_complete(_test())
    ...
    crawler = Crawler(
        storage=JSONRuleStorage.loads(
            r'{"peps.python.org": {"host": "peps.python.org", "crawler_rules": {"list": {"name":"list","request_args":{"method":"get","retry":3,"timeout":8,"url":"https://peps.python.org/","headers":{"User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/79.0.3945.130 Safari/537.36"}},"parse_rules":[{"name":"list","chain_rules":[["css","td:nth-of-type(3) > a.pep","@href"],["re","^/","@https://www.python.org/"],["python","getitem","[:3]"]],"childs":""}],"regex":"^https://peps.python.org/$"}, "detail": {"name":"detail","request_args":{"method":"get","retry":3,"timeout":8,"url":"https://peps.python.org/pep-0001/","headers":{"User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/79.0.3945.130 Safari/537.36"}},"parse_rules":[{"name":"title","chain_rules":[["css","h1.page-title","$text"],["python","getitem","[0]"]],"childs":""}],"regex":"^https://peps.python.org/pep-\\d+/$"}}}}'
        )
    )
    result = crawler.crawl("https://www.python.org/")
    # print(result)
    assert isinstance(result, RuleNotFoundError)
    assert str(result) == "No rule matched the given url: https://www.python.org/"
    crawler = Crawler(
        storage=JSONRuleStorage.loads(
            r'{"clericpyclericpyclericpyclericpy.com.cn": {"host": "clericpyclericpyclericpyclericpy.com.cn", "crawler_rules": {"list": {"name":"list","request_args":{"method":"get","retry":3,"timeout":8,"url":"https://clericpyclericpyclericpyclericpy.com.cn/dev/peps/","headers":{"User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/79.0.3945.130 Safari/537.36"}},"parse_rules":[{"name":"list","chain_rules":[["css","td:nth-of-type(3) > a.pep","@href"],["re","^/","@https://clericpyclericpyclericpyclericpy.com.cn/"],["python","getitem","[:3]"]],"childs":""}],"regex":"^https://clericpyclericpyclericpyclericpy.com.cn/dev/peps/$"}}}}'
        )
    )
    result = crawler.crawl("https://clericpyclericpyclericpyclericpy.com.cn/dev/peps/")
    # print(result)
...
def test_uni_parser_frequency():
    def test_sync_crawl():
        ...
        test_count = 3
        tasks = [pool.submit(uni.download, crawler_rule) for _ in range(test_count)]
    ...
    test_sync_crawl()
    global_loop.run_until_complete(test_async_crawl())
...
def test_utils():
    ...
    assert (
        result
        == '<a href="http://www.abc.com/b">test</a><a href="http://www.abc.com/a/b/c/d/b">test</a><a href="http://www.abc.com/a/b/c/b">test</a><a href="http://www.abc.com/a/b/b">test</a><img src="http://www.abc.com/b"><img src="http://www.abc.com/a/b/c/d/b"><img src="http://www.abc.com/a/b/c/b"><img src="http://www.abc.com/a/b/b">'
    )
